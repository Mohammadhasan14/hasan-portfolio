  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-admin-display text-[22px] font-semibold text-admin-text">Projects</p>
          <p className="font-admin-mono text-[11px] text-admin-faint">{(projects ?? []).length} total</p>
        </div>
        <Link href="/admin/projects/new" className={`${primaryButtonClass} w-auto`}>
          + Add project
        </Link>
      </div>
      <div className="mt-6 flex flex-col gap-3">
        {(projects ?? []).map((proj) => (
          <div key={proj.id} className={`${cardClass} flex gap-3 p-4`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1.5 font-admin-mono text-[11px] tracking-wide text-admin-faint">
                {proj.code} · sort {proj.sort_order} · {proj.status_label}
              </p>
              <div className="mt-3 flex gap-2">
                ...
                <ConfirmDeleteButton
                  action={deleteProject.bind(null, proj.id, proj.name)}
                  itemName={proj.name}
                />
              </div>
            </div>
        ...
        {(projects ?? []).length === 0 && <p className="text-sm text-admin-muted">No projects yet.</p>}
      </div>
